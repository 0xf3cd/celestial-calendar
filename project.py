...
def parse_args() -> argparse.Namespace:
  ...
  def cores(value):
    ...
    try:
      ...
      raise argparse.ArgumentTypeError(f'Invalid value for --cores: {value}. Must be an integer or "all".') from e
...
def create_tasks(args: argparse.Namespace) -> List[Task]:
  ...
  if args.cmake:
    tasks.append(Task("Run CMake", lambda: run_cmake(BUILD_VERSION, build_type=args.build_type)))
  if args.build:
