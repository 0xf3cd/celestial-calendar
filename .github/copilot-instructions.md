...
- **Toolchain**: C++23, clang++ / g++; the versions CI pins live in `.github/workflows/`. `clang-tidy` runs `WarningsAsErrors: '*'` — zero warnings tolerated. Respect the deliberate `.clang-tidy` disables instead of "fixing" code to satisfy them.
