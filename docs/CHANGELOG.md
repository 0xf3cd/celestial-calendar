...
- JavaScript validates shape, finiteness, integer width, and each operation's domain before entering WASM. Bad input throws `TypeError` / `RangeError`; native failures become `CelestialError`, and only the 28 recording C exports may contribute a native message. Legitimate absence remains `null` or `[]`.
