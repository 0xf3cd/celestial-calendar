...
and `last_error` stay internal. `python3 toolbox/build_npm.py` stages and packs the exact nine-file npm tarball
...
raw `.mjs/.wasm` pair, `LICENSE`, `THIRD_PARTY_NOTICES.txt`, the exact npm tarball, `npm-pack.json`, and a SHA-256 sidecar. The release flow publishes
