...
9. **CI toolchains are pinned — every one that can emit a diagnostic.** A tool that updates
   ...
   **20** on Windows (not the 18 the Linux legs use — a pre-existing split these pins record
   ...
   binary by its major, and the vendored `run-clang-tidy.py` is the matching `llvmorg-22.1.2` —
   an 18 runner driving a 22 binary reports `No checks enabled` and zero findings, which on that
   leg is indistinguishable from a clean tree. Exact versions live in the workflows,
...
| **The `using X = Y` aliases in the lunar headers stay** (`common.hpp` / `converter.hpp` / `algo2.hpp`) | They are alias-declarations, and the style rule's closed set allows that form without conditions — the "imports a named template" clause governs using-*declarations*, a different construct that injects a foreign name instead of introducing a local one. `datetime.hpp`, the example the rule itself points at, writes seven of them; the repo has twenty-three. They are also load-bearing: `lunar/common.hpp` and `lunar/converter.hpp` do not include `datetime.hpp`, so deleting their alias either reddens the self-contained gate or forces a new include — a worse outcome than the tidiness it was meant to buy | The style rule narrows what `using X = Y` may do | 2026-08-09 |
...
| **`normalize_deg` / `normalize_rad` keep calling `std::remainder`, and stay `constexpr`** | Standards-compliant is not the same as usable: P0533 made `std::remainder` constexpr in C++23 and no standard library we target has shipped it. Measured across the three, one machine, one set of libstdc++ 15 headers: `g++-15` accepts, `g++-15 -fno-builtin` rejects, and clang 21 and 22 both reject naming glibc's declaration — so the accepting case is GCC folding `remainder` as a builtin, not libstdc++ implementing P0533 (its `<cmath>` re-exports glibc's non-constexpr one with `using ::remainder;`). Apple's SDK rejects for its own reasons and the UCRT structurally (`remainder` is an extern `_ACRTIMP`). Every leg we build with is a clang or MSVC leg, so every one of them rejects. Four constexpr entry points, two direct call sites, all IF-NDR — recorded rather than papered over (#82) | Any target standard library marks `std::remainder` constexpr. `automation/feature_probe.py` watches it per leg, so this fires as a red build rather than as someone remembering to re-run a probe. The previous trigger — "a pinned toolchain bump" — fired across four major versions in 2026-08 and changed the answer by nothing | 2026-08-05, premise and trigger refreshed 2026-08-09 |
