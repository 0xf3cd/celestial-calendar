...
| **The four `shutil.which` checks in `automation/` are not merged into one** | They share one line; what differs is what each does when the tool is missing — return `None` so the gate goes red (`abi_layout`), raise so the caller records a failure (`export_surface`), or report and carry on because stopping is `--setup`'s call (`sysinfo`). One shared helper has to pick one of those three for all of them. Separately, the four `sys.path.append` copies in `toolbox/` cannot be merged at all: a shared bootstrap module would itself need the path fixed before it could be imported (#166) | A third call site needs the same failure policy as an existing two | 2026-08-08 |
